...
void test_bfgs_hessian_update_cpu_vs_gpu() {
    ...
    // Tolerance for Hessian update (1e-2 accounts for float precision differences
    // between CPU and GPU implementations with different operation ordering)
    BOOST_REQUIRE_SMALL(max_diff, (float)1e-2);
